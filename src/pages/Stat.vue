...
const getStatWeek = () =>
  axios
    .get(
      `${import.meta.env.VITE_APP_API_URL}sadhanas/myjournal/${store.user.id}/${
        week.value
      }.json`,
      store.tokenHeader
    )
...
<template>
  <section>
    ...
    <div class="grid-container">
      <span
        class="grid-row"
        v-for="(row, index) in weekData.japa"
        v-bind:class="{ row0: index == 0 }"
      >
        <span
          class="grid-column"
          v-for="(column, index) in row"
          v-bind:class="{ column0: index == 0 }"
          ><span v-if="column == 0"></span
          ><span v-else>{{ column }}</span></span
        >
      </span>
    </div>
    <div class="title">
